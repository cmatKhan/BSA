...
for (i in pools) {
  # Inoculum
  # # this is the basis for the table; the four first columns of mySamples.
  meta <- mySamples[[grep(pattern = i, x = Inoculum, value = T)[1]]][, 1:4] 
  meta$RealDepth <- rowSums(
    sapply(grep(pattern = i, 
                x = Inoculum, 
                value = T), 
           FUN = function(x) { 
             cbind(mySamples[[x]][, 8])})
