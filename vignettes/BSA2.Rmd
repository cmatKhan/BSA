...
replicatesBSA = foreach(
  row = iterators::iter(sample_comparison_frame(pool_meta),by='row')
) %do% {
    picker2(samples_df,
            lowBulk = row[['lowBulk']],
...
# pooled BSA
do the same thing with the picker2, but this time on the data set with 
...
for (i in c("Y", "L", "B")) {
  lowbulk <- "I"
  highbulk <- i
  allPoolsInOneComparisonBSA[[i]] <- 
    picker2(allPoolsInOneBSA, 
            lowBulk = lowbulk, 
            highBulk = highbulk)
  print(paste("Comparisons made:", highbulk, "x", lowbulk, sep = " "))
}
